package kr.co.ict;
// SQL관련된 요소는 java.sql.*을 임포트하면 대부분이 다 로드됩니다.
import java.sql.*;

public class JDBCSelcet {

	public static void main(String[] args) {
		// JDBC 연결 여부 확인
		
		try {
			// MYSQL을 연동할 것을 알리기 위해 forName 내부에 MySQL용 연동구문을 적습니다.
			...
			// SELECT 문을 실행한 결과 자료는 ResultSet이라는 자료로 받을 수 있씁니다.
			ResultSet rs = stmt.executeQuery("SELECT * FROM userinfo");
			// stmt.executeUpdate()는 SELECT이외의 구문, INSERT와 UPDATE를 실행할 때 씁니다.

			// ResultSet은 기본적으로 row개수만큼 내부에 데이터를 저장합니다.
			// ResultSet은 특정 번호를 집어서 조회하는 경우보다는 순차적으로
			// 조회해서 쓰는 경우가 많습니다.
			// 맨 처음 ResultSet은 -1번이라는 임시번호를 타겟으로 잡습니다.
			// 이 번호를 옯기기 위해 .next()를 호출하면 다음 번호로 넘어갑니다.
			// rs.next(); // 다음 번호가 조회되면 true, 없으면 false
			// get 자료형(인덱스번호, 1부터) or get 자료형(컬럼명)을 쓰며
			// 해당 컬럼의 자료를 반환합니다.
			...
			// 콘솔에 찍어주세요.
			// 1, 4번은 컬럼명으로 출력, 2, 3번 컬럼은 인덱스로 출력
